...
class CagadaForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'hora_inicio_cagada': forms.DateTimeInput(attrs={'type': 'datetime-local'}),
            'hora_final_cagada': forms.DateTimeInput(attrs={'type': 'datetime-local'}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if usuario is not None:
            self.fields['ubicacion'].queryset = Ubicacion.objects.filter(usuario=usuario)

    def clean(self):
        cleaned_data = super().clean()
        hora_inicio = cleaned_data.get('hora_inicio_cagada')
        hora_final = cleaned_data.get('hora_final_cagada')
